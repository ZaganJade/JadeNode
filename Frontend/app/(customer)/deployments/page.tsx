...
export default function DeploymentsPage() {
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <div>
          <div className="h-8 w-52 animate-pulse rounded bg-[rgba(255,191,0,0.08)]" />
          <div className="mt-2 h-4 w-80 animate-pulse rounded bg-[rgba(255,191,0,0.05)]" />
        </div>
        <div className="space-y-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-32 animate-pulse rounded-2xl bg-[rgba(255,191,0,0.04)]"
            />
          ))}
        </div>
      </div>
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-[#F5F5F0]">Deployments</h1>
        </div>
                ...
                <div className="rounded-2xl border border-[rgba(255,191,0,0.08)] bg-[rgba(25,20,0,0.4)] p-6 backdrop-blur-[24px] transition-colors hover:border-[rgba(255,191,0,0.18)]">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-[#F5F5F0]/40">
                      ...
                      {deployment.period_start && deployment.period_end && (
                        <div className="mt-1 flex items-center gap-1 text-[10px] text-[#F5F5F0]/25">
                    ...
                    {/* Right: arrow */}
                    <svg
                      className="h-5 w-5 shrink-0 text-[#F5F5F0]/15 transition-transform group-hover:translate-x-1 group-hover:text-[#FFBF00]/60"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={2}
